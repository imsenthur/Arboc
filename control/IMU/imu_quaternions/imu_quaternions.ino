...
// defining quaternion container for the imus
double quaternion[Number_of_IMU][4] = {0};
...
// =====================================================================
// ===               Euler TO Quaternion ROUTINE                     ===
// =====================================================================

// Function to convert euler angles to quaternion angles 
void euler_to_quaternion(double angle1, double angle2, double angle3,int imu_number)
{
  quaternion[imu_number][0] = cos(theta[imu_number][0]/2) * cos(theta[imu_number][1]/2) * cos(theta[imu_number][2]/2) - sin(theta[imu_number][0]/2) * sin(theta[imu_number][1]/2) * sin(theta[imu_number][2]/2);
  quaternion[imu_number][1] = sin(theta[imu_number][0]/2) * cos(theta[imu_number][1]/2) * cos(theta[imu_number][2]/2) + cos(theta[imu_number][0]/2) * sin(theta[imu_number][1]/2) * sin(theta[imu_number][2]/2);
  quaternion[imu_number][2] = cos(theta[imu_number][0]/2) * sin(theta[imu_number][1]/2) * cos(theta[imu_number][2]/2) - sin(theta[imu_number][0]/2) * cos(theta[imu_number][1]/2) * sin(theta[imu_number][2]/2);
  quaternion[imu_number][3] = cos(theta[imu_number][0]/2) * cos(theta[imu_number][1]/2) * sin(theta[imu_number][2]/2) + sin(theta[imu_number][0]/2) * sin(theta[imu_number][1]/2) * cos(theta[imu_number][2]/2);
}

// =====================================================================
// ===               Quaternion To Euler ROUTINE                     ===
// =====================================================================

// Function to convert quaternion to euler angles
void quaternion_to_euler(double q1, double q2, double q3, double q4,int imu_number)
{
  theta[imu_number][0] = atan2(2 * (q2 * q3 + q3 * q1),q2 * q2 - q3 * q3 - q4 * q4 + q1 * q1);
  theta[imu_number][1] = asin(2 * (q2 * q4 - q3 * q1)/(q2 * q2 + q3 * q3 + q4 * q4 + q1 * q1));
  theta[imu_number][2] = atan2(2 * (q4 * q3 + q2 * q1),-q2 * q2 - q3 * q3 + q4 * q4 + q1 * q1);
}

// =====================================================================
// ===             Quaternion Multiplication ROUTINE                 ===
// =====================================================================

// Function to convert euler angles to quaternion angles 
void relative_angle(double p[4], double q[4] , int imu_number)
{
  double r[4] = {0};
  for(int i=1;i<4;i++) q[i] = q[i] * -1;  // Congugate of quaternion 2
  r[0] = p[0] * q[0] - p[1] * q[1] - p[2] * q[2] - p[3] * q[3];
  r[1] = p[0] * q[1] + p[1] * q[0] + p[3] * q[2] - p[2] * q[3];
  r[2] = p[0] * q[2] - p[3] * q[1] + p[2] * q[0] + p[1] * q[3];
  r[3] = p[0] * q[3] - p[1] * q[2] + p[2] * q[1] + p[3] * q[0]; 
  quaternion_to_euler(r[0],r[1],r[2],r[3] , imu_number);
}
...
void getDataFromMPU()
{
  for(int i = 0 ;i < Number_of_IMU ; i++)
  {
    ...
    */
    ...
    euler_to_quaternion(theta[i][0],theta[i][1],theta[i][2],i);
...
{ 
  ...
  for(int i = 1 ;i < Number_of_IMU ; i++)
  {
     ...
     */
    // Relative angle between two quaternions is q1/q2 = q1 * q2 ^-1 = q1 * conjugate(q2)
    // Order is important as quaternion multiplication is not commutative
    relative_angle(quaternion[i] , quaternion[i-1] , i);
    Serial.print("MPU ");Serial.print(i);Serial.print(" with respect to MPU ");Serial.println(i-1);
    Serial.print("RelativeTheta X = "); Serial.print(theta[i][0]);
    Serial.print(" | RelativeTheta_Y = "); Serial.print(theta[i][0]);
    Serial.print(" | RelativeTheta_Z = "); Serial.println(theta[i][0]);
